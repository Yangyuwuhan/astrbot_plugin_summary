...
class Downloader:
    ...
    async def streamd(
        ...
        *,
        ...
        file_path = self.cfg.temp_dir / file_name
        # 如果文件存在，则直接返回
        if file_path.exists():
            return file_path
    ...
    async def ytdlp_download_video(
        ...
        *,
        ...
        video_path = self.cfg.temp_dir / generate_file_name(url, ".mp4")
    ...
    async def ytdlp_download_video_relaxed(
        ...
        *,
        ...
        video_path = self.cfg.temp_dir / f"{file_stem}.mp4"
        if video_path.exists():
            return video_path

        opts = {
            "outtmpl": str(self.cfg.temp_dir / file_stem) + ".%(ext)s",
            "merge_output_format": "mp4",
        ...
        candidates = sorted(self.cfg.temp_dir.glob(f"{file_stem}*.mp4"))
        if candidates:
            return candidates[0]
    ...
    async def ytdlp_download_audio(
        ...
        *,
        ...
        file_name = generate_file_name(url)
        audio_path = self.cfg.temp_dir / f"{file_name}.flac"
        if audio_path.exists():
            return audio_path

        opts = {
            "outtmpl": str(self.cfg.temp_dir / file_name) + ".%(ext)s",
